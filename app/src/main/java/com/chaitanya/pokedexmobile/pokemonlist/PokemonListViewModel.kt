...
import com.chaitanya.pokedexmobile.ui.Constants.PAGE_SIZE
...
class PokemonListViewModel @Inject constructor(private val repository: PokemonRepository) :ViewModel() {
            ...
            when(result) {
                is Resource.Success -> {
                    ...
                    val pokedexEntries = result.data.results.mapIndexed { _, entry ->
                        ...
                        PokedexListItem(entry.name.replaceFirstChar {
    ...
    fun calcDominantColor(drawable: Drawable,onFinish : (Color)-> Unit){
        viewModelScope.launch {
            ...
            Palette.from(bitmap).generate{palet ->
                palet?.dominantSwatch?.rgb?.let {
                    onFinish(Color(it))
                }
            }
        }
